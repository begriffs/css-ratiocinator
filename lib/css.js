...
var CSS = (function () {
    ...
    var points = _.reduce(
      ...
      function (list, q) {
        return list.concat(_.compact(
          _.map(
            ['min-width', 'max-width'],
            function (prop) { return +(q.match(prop + '\\s*:\\s*(\\d+)') || [])[1]; }
          )
        ));
      },
      [0]
    );
    return _.uniq(points.sort(function (a, b) { return a - b; }), true);
  };
  ...
  my.renderStyle = function (properties, selector, indent) {
    var css = '';
    indent = indent || '';
  ...
  my.mediaWidthIntervals = function () {
    var points  = my.mediaBreakpoints(),
      intervals = _.filter(
        _.zip(points, points.slice(1).concat([Infinity])),
    ...
    return _.map(
      ...
      function (I) {
        return {
          min: I[0],
          max: I[1],
          sample: I[1] !== Infinity ? I[0] + (I[1] - I[0]) / 2 : I[0] + 1
        };
      }
    );
  };
