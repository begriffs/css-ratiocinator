...
(function () {
  ...
  function importance(choice) {
    var number = $('html').find(choice.selector).length,
      size = _.keys(choice.style).length,
      selector_length = (choice.selector.match(/ /) || []).length;
    return -(number * number * size) + selector_length;
  }
  ...
  function fontsToEm(root, base_size) {
    _.each(['font-size', 'line-height'], function (prop, i) {
      ...
      if (val) {
        match = val.match(/px$/);
        if (match) {
          root.data('style')[prop] = (val.match(/\d+/)[0] / base_size).toFixed(3) + 'em';
        }
      }
    });
  ...
  window.simplerStyle = function () {
    ...
    delete root.data('style')['font-size'];
